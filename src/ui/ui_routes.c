...
static esp_err_t locations_get_handler(httpd_req_t *req)
{
    httpd_resp_set_type(req, "text/html");
    httpd_resp_set_hdr(req, "Cache-Control", "no-store");
    return httpd_resp_send(req, UI_LOCATIONS_HTML, HTTPD_RESP_USE_STRLEN);
}
...
static const httpd_uri_t uri_locations = {.uri = "/locations", .method = HTTP_GET, .handler = locations_get_handler};
